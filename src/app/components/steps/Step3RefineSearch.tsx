...
export const Step3RefineSearch: React.FC<Step3RefineSearchProps> = ({
  ...
  React.useEffect(() => {
    onValidate(
      selectedCategories.length > 0 || selectedCompetitions.length > 0
    );
    ...
    content = (
      <Box sx={{ py: 5, textAlign: "center" }}>
        <Typography variant="h6" color="warning.main">
          No active schedules found for your selection today. Please go back and
      ...
      <Box>
        <Autocomplete
          ...
          renderInput={(params) => (
            <TextField
              {...params}
              label="Filter by Countries/Categories (Max 5)"
              placeholder="e.g., International, Spain, Germany"
            />
          )}
          sx={{ mb: 4 }}
        />
        ...
        <Autocomplete
          ...
          renderInput={(params) => (
            <TextField
              {...params}
              label="Filter by Leagues/Competitions (Max 5)"
              placeholder="e.g., Champions League, Premier League, Superliga"
            />
          )}
        />
      </Box>
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Typography variant="body1" color="text.secondary" sx={{ mb: 4 }}>
        Based on today's schedule for your selected sports, choose up to{" "}
        {MAX_SELECTIONS} categories or competitions to focus on.
      </Typography>
